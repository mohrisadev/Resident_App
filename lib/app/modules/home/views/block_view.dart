...
class BlockView extends GetView<HomeController> {
  const BlockView({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    final Size size = MediaQuery.of(context).size;
    controller.getListofBlocks();
      ...
      return LoadingOverlay(
          ...
          child: Scaffold(
              appBar: AppBar(
                ...
                title: !controller.isSearching
                    ...
                    : TextField(
                        ...
                        controller: controller.blockCommunityController,
                        style: AppConstants.appStyles.hintTopbar,
                        decoration: InputDecoration(
                            icon: Icon(Icons.search, color: Colors.white),
                            hintText: 'Enter a Block',
              ...
              body: Column(children: [
                Expanded(
                  child: getBlockListWidget(),
                ),
              ])));
    });
  }

  Widget getBlockListWidget() {
    if (controller.filteredblockItems.isNotEmpty) {
      return Container(
          width: double.maxFinite,
          padding: EdgeInsets.all(2),
          child: ListView(children: [
            for (var item in controller.filteredblockItems)
              Card(
                child: InkWell(
                    splashColor: Colors.orange[600],
                    onTap: () async {
                      controller.setBlock(item);
                    },
                    child: ListTile(
                        dense: true,
                        title: Text('${item['block_name']}',
