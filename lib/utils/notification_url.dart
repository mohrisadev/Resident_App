...
Future<void> showNotificationWithIconsAndActionButtons(
  ...
  await AwesomeNotifications().createNotification(
      ...
      actionButtons: [
        NotificationActionButton(
            key: 'APPROVE', label: 'Approve', autoDismissible: true),
        NotificationActionButton(
            key: 'REJECT', label: 'Reject', autoDismissible: true)
      ]);
}
...
Future<void> yellowNotification(int id, bool delayLEDTests) async {
  AwesomeNotifications().setChannel(NotificationChannel(
      ...
      channelDescription: "A red colorful notification",
      vibrationPattern: lowVibrationPattern,
      defaultColor: AppConstants.appcolors.appOrange,
      ledColor: AppConstants.appcolors.appOrange,
      ledOnMs: 1000,
      ledOffMs: 500));
...
Future<void> listScheduledNotifications(BuildContext context) async {
  ...
  for (NotificationModel schedule in activeSchedules) {
    debugPrint(
        'pending notification: [id: ${schedule.content!.id}, title: ${schedule.content!.titleWithoutHtml}, initial: ${schedule.schedule.toString()} ]');
  }
